...
async function createRequest(request, action, dispatch, payload, options) {
    ...
    try {
        ...
        if (encryptedGateway) {
            // eslint-disable-next-line no-undef
            const requestDataType = EUSCClient.DataType[(requestDataTypes[action] && requestDataTypes[action].request) || 'String'];
            // eslint-disable-next-line no-undef
            const responseDataType = EUSCClient.DataType[(requestDataTypes[action] && requestDataTypes[action].response) || 'String']

            // eslint-disable-next-line no-undef
            const euRequest = new EUSCRequest(
                url,
                method.toUpperCase(),
                Object.keys(requestData.headers).map(header => [header, requestData.headers[header]].join(': ')),
                requestDataType,
                request.body,
                responseDataType
            );
            ...
            response.blob = () => new Blob([response.data], { type: response.headers.get('content-type') || 'application/octet-stream' });
        ...
        const responseData = 'data' in responseBody ? objectProxy(responseBody.data, { headers: response.headers }) : responseBody;

        if (responseBody.meta || responseBody.pagination) {
            responseData.meta = responseBody.pagination || responseBody.meta;
        }

        dispatch({ type: `${action}_SUCCESS`, payload: responseData, request: payload, url, method, body: request.body });
        return responseData;
    } catch (error) {
