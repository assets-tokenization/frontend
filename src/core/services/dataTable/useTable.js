...
const useTable = ({
    ...
    const tableInitialState = (sticky && JSON.parse(storage.getItem('useTable' + sourceName))) || initialState;
    ...
    const fetchFunc = async (params = {}) => {
        const url = (getDataUrl || composeUrl)(dataURL, { ...state, ...params });
        ...
        switch (method) {
            case 'POST': {
                ...
                data = await api.post(url, requestBody, dispatchType(sourceName, GET_LIST), store.dispatch);
                break;
            }
            case 'GET':
            default:
                data = await api.get(url, dispatchType(sourceName, GET_LIST), store.dispatch)
    ...
    const loadAllDataRequests = (additional = {}) => {
        ...
        return Array(Math.ceil(state.count / rowsPerPage))
            .fill(null)
            .map((n, page) => () => fetchFunc({ ...additional, page: page + 1 }));
    };
    ...
    const onRowsDelete = async (rowsDeleted, forceLoad = true, permanent = false) => {
        try {
            state.loading = true;
            await promiseChain(
                rowsDeleted.map(rowId => () => {
                    ...
                    return api.del(deleteParams.join('/'), {}, dispatchType(sourceName, actionType), store.dispatch);
                })
            );
            state.loading = false;
            forceLoad && load();
    ...
    const onRowsRecover = async (rowsDeleted, forceLoad = true) => {
        try {
            state.loading = true;
            await promiseChain(
                rowsDeleted.map(rowId => () => api.post([getURL(false), rowId, 'recover'], {}, dispatchType(sourceName, ON_ROWS_RECOVER), store.dispatch))
            );
            state.loading = false;
            forceLoad && load();
        } catch (e) {
    ...
    const endPointActions = Object.keys(actions || {})
        .reduce((acc, action) => ({
            ...acc,
    ...
    return {
        ...
        actions: {
            ...
            onRowsDelete,
            onRowsDeletePermanent: (rowsDeleted, forceLoad) => onRowsDelete(rowsDeleted, forceLoad, true),
            onRowsRecover,
