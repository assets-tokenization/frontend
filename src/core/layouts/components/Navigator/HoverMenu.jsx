...
import { ListItem, ListItemIcon, ListItemText, Badge, Menu } from '@mui/material';
...
const HoverMenu = ({ t, classes, menuItem, userUnits, userInfo }) => {
    ...
    return (
        <>
            ...
            >
                {children.filter(checkItemAccess).map((child, key) => <Item key={key} menuItem={child} noPadding={true} />)}
            </Menu>
        </>
    );
};
...
const styled = withStyles(theme => ({ ...itemStyles(theme), ...menuStyles }))(HoverMenu);
