...
const onMessage = async function ({ data: { commandId, pageDataOrigin, schema = {}, documentData } }) {
    ...
    propertiesEach(schema, pageData, (propertySchema, propertyData, propertyPath, parentSchema, parentData, propertyKey) => {
        const checkFunction = (func) => {
            ...
            if ((parentSchema.control === 'form.group' && parentSchema.useOwnData) || parentSchema.useOwnDataArray) {
                let newPath = propertyPath.split('.').filter(Boolean);
                const numberOfElementsToRemove = 2;
                newPath = newPath.length > numberOfElementsToRemove ? newPath.slice(0, newPath.length - numberOfElementsToRemove).join('.').replace(/\[(\d+)\]/g, '.$1') : '';
    ...
    self.postMessage({ commandId, result: normalizeErrors(checkFunctionErrors.concat(validator.errors).filter(Boolean)) })
};
