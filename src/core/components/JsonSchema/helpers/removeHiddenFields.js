...
const removeHiddenFields = (schema, data, { stepData = {}, parentData } = {}) => {
    ...
    const properties = Object.keys(schemaProperties).reduce((acc, propertyName) => {
        ...
        if (property.checkHidden && evaluate(property.checkHidden, value, stepData, data, parentData || {}) === true) {
            if (required.includes(propertyName)) {
                required.splice(required.indexOf(propertyName), 1);
            }
            return acc;
        ...
        return {
            ...acc,
            [propertyName]: property.type === 'object' ? removeHiddenFields(property, data, { stepData, parentData: value }) : property
        };
    }, {});
