...
export default ({
    ...
    if (hidden && typeof hidden === 'string') {
        try {
            const result = evaluate(hidden, rootDocument.data, value, parentValue, userInfo);
            return result === true;
        } catch (e) {
    ...
    if (checkHidden && typeof checkHidden === 'string') {
        try {
            const result = evaluate(checkHidden, value, rootDocument.data[steps[activeStep]], rootDocument.data, parentValue, userInfo);
            return result === true;
        } catch (e) {
