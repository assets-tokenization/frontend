...
const SchemaForm = (props) => {
    ...
    React.useEffect(() => {
        if (setDefaultValue && control !== 'register' && value === undefined && !defaultValueExecuted.includes(stringifiedPath)) {
            try {
    ...
    }, [actions, control, defaultValueExecuted, onChange, stringifiedPath, rootDocument.data, setDefaultValue, value]);
    ...
    React.useEffect(() => {
        ...
        if (newIsHidden !== isHidden || (value !== null && value !== undefined)) {
            ...
            if (value !== null && value !== undefined && newIsHidden && cleanWhenHidden && !keepSelection) {
                waiter.addAction('deleteAttaches-' + stringifiedPath, async () => {
                    if (schema?.control === 'select.files') {
                        const controlData = objectPath.get(rootDocument.data, `${steps[activeStep]}.${stringifiedPath}`);
    ...
    }, [activeStep, isHidden, parentValue, rootDocument, schema, steps, value, cleanWhenHidden, keepSelection, onChange, stringifiedPath, userInfo, dispatch]);
    ...
    const withTooltip = (() => {
        try {
            const isSupported =
                SUPPORTED_TOOLTIP_LISTS.concat(SUPPORTED_TOOLTIP)
                    .some((supported) => [schema?.type, schema?.control].includes(supported)) &&
                !UNSUPPORTED_TOOLTIP
                    .some((supported) => [schema?.control].includes(supported));

            if (!isSupported) return;

            const evaluatedTitle = evaluate(schema?.description, value, rootDocument.data[steps[activeStep]], rootDocument.data);
    ...
    const tooltipPosition = SUPPORTED_TOOLTIP_LISTS.includes(schema?.control) ? 'top-start' : 'bottom-start';
