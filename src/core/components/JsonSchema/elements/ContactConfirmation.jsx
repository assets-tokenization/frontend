...
class ContactConfirmation extends React.Component {
  ...
  render() {
    ...
    const { t, classes, description, readOnly, width, path, value, autoFocus, errors, actions } =
      this.props;
    ...
    if (handlingError) {
      return <BlockQuote variant="error" title={t(handlingError.message, { contact })} />;
    }
    ...
    if (value.confirmed) {
      ...
      return (
        <BlockQuote
          variant="success"
          title={t([type, 'confirnmation', 'success'].map(capitalizeFirstLetter).join(''))}
        />
      );
    }
    ...
    return (
      ...
      >
        <BlockQuote
          variant="warning"
          title={t([type, 'confirnmation'].map(capitalizeFirstLetter).join(''), { contact })}
        />
        <TextField
          variant="standard"
          autoFocus={autoFocus}
