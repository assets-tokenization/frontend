...
const UserList = ({
    ...
    const [userList, setUserList] = React.useState(Object.values(value || {}).filter(Boolean));

    React.useEffect(() => {
        if (!_.difference(value, userList).length && !_.difference(userList, value).length) {
            return;
        }
    ...
    React.useEffect(() => {
        waiter.addAction('user-list-search', async () => {
            ...
            if (autocompleteFilters) {
                ...
                const isIpn = /^\d{8}$/.test(searchValue) || /^\d{10}$/.test(searchValue);
                const isId = searchValue.length === 24 && searchValue.split(' ').length === 1;
            ...
            const results = await actions.searchUsers(searchParams, '?brief_info=true');
    ...
    return <>
            ...
            autocompleteFilters ? null : (
                <Toolbar disableGutters={true}>
                    <TextField
                        ...
                        value={filter}
                        style={{ flexGrow: 1 }}
                        onChange={({ target: { value: newFilter } }) => setFilter(newFilter)} />
                </Toolbar>
            )
        }
        ...
        >
            <Toolbar className={classes.header}>
                ...
                <IconButton className={classes.button} onClick={() => setOpen(false)} size="large">
                    <CloseIcon />
                </IconButton>
            </Toolbar>
...

                {
                    autocompleteFilters ? null : (
                        <Select
                            variant="standard"
                            value={searchBy}
                            onChange={({ target: { value: newSearchBy } }) => setSearchBy(newSearchBy)}>
            ...
            <div className={classes.resultsWrapper}>
                {
                    triggered ? (
                        <UserTable
                            ...
                            data={searchResults && searchResults.filter(({ userId }) => !userList.includes(userId))}
                            shortInfo={true}
                            UserAction={
                                ({ userId }) => (
                                    <>
