...
const fiterProps = (tabs, props = {}, currentTab) => {
    ...
    const filterForTabs = tabs
        ? keys.filter(key => (props[key].tabsIds ? !!props[key].tabsIds.find(id => id === currentTab) : true))
...
class SelectUser extends React.Component {
    ...
    handleSelect = (user) => {
        ...
        const values = Object.keys(properties).reduce((acc, key) => ({ ...acc, [key]: user[key] }), {
            encodeCertSerial,
            encodeCert,
            isLegal
        });

        this.setState({ openSelectUserDialog: false }, () => onChange && onChange(values));
    };
    ...
    handleChange = name => (value) => {
        ...
        // if (name === 'isLegal') {
        //     const { isLegal, ...values } = this.props.value || {};
        //     const properties = this.props.properties || {};
    ...
    renderElement = (key, index) => {
        ...
        const { t, classes, value, signer, schema, path, properties, ...rest } = this.props;
                ...
                {(!index && signer) ? (
                    <div className={classes.buttonContainer}>
                        <IconButton
                            ...
                            onClick={() => this.setState({ openSelectUserDialog: !openSelectUserDialog })}
                        >
                            <Search />
                        </IconButton>
                    </div>
    ...
    renderTabs() {
        ...
        return tabs && tabs.map(tab => (
            <Chip
                ...
                onClick={(!readOnly ? () => this.handleChange('isLegal')(tab.id === 2) : null)}
                style={{
