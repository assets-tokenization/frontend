...
const getFilters = ({ rootDocument, value, steps, activeStep, filters = [] }, search) => filters.reduce((data, item) => {
    const result = evaluate(item.value, value, rootDocument.data[steps[activeStep]], rootDocument.data, search);
...
const ExternalRegister = props => {
    ...
    const toggleBusy = React.useCallback(async bool => {
    ...
    React.useEffect(() => {
        const init = async () => {
            try {
                ...
                const newOptions = !originDocument.isFinal && !hidden ? await processList.hasOrSet('requestExternalData', registerActions.requestExternalData, externalReaderRequestData) : [];
                ...
                setOptions(autocomplete ? uniq(options.concat(newOptions.map(toOption))) : newOptions.map(toOption));
            } catch (e) {
    ...
    return hidden ? null : (
        ...
        >
            <Select
                ...
                onChange={selected => onChange(new ChangeEvent(selected, true, true, true))}
...
const mapDispatch = dispatch => ({
    registerActions: {
        ...
        requestExternalData: requestData => api.post('external_reader', requestData, 'REQUEST_EXTERNAL_DATA', dispatch)
    }
});
