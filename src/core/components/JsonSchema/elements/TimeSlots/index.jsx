...
const styles = {
    ...
    chip: {
        ...
        '&:hover': {
            ...
            backgroundImage: 'linear-gradient(217deg,rgba(255,0,0,.8),rgba(255,0,0,0) 70.71%),linear-gradient(127deg,rgba(0,0,255,.8),rgba(0,0,255,0) 70.71%),linear-gradient(336deg,rgba(0,255,0,.8),rgba(0,255,0,0) 70.71%)'
        },
    },
    chipContent: {
        borderRadius: 50,
...
const TimeSlots = (props) => {
    ...
    const dateSorceValue = objectPath.get(rootDocument.data, dateSorce);
    const dataSorceValue = objectPath.get(rootDocument.data, dataSorce);

    if (!dataSorceValue) return null;

    const timeList = dataSorceValue
        .map(({ date, times }) => {
            ...
            const filterActive = (times || []).filter(({ status, timeStart }) => {
                const evalated = checkActive ? evaluate(checkActive, `${day}T${timeStart}:00`, rootDocument.data) : false;
                return Number(status) || evalated;
            });
    ...
    const timeValuesToday = timeList.filter(({ day }) => moment(day, 'YYYY-MM-DD').format('DD.MM.YYYY') === dateSorceValue);
    const timeValuesFuture = timeList.filter(({ day }) => moment(day, 'YYYY-MM-DD').format('DD.MM.YYYY') !== dateSorceValue);
                    ...
                    <>
...

                        {
                            !timeValuesToday.length && timeValuesFuture.length ? (
                                <>
                                    <Typography className={classNames(classes.subHeading, classes.mt70)}>
                                        {t('FreeSlots')}
                                    </Typography>
...

                                    {
                                        timeValuesFuture.map(({ toPrint, times, day }) => (
                                            <div key={toPrint} className={classes.mt54}>
                                                <ExpansionPaper
                                                    title={toPrint}
                                                    defaultExpanded={moment(value?.date).format('YYYY-MM-DD') === day}
                                                >
                                                    <div className={classes.chipWrapper}>
                                                        {times.map((item) => renderChip(item, day))}
                                                    </div>
