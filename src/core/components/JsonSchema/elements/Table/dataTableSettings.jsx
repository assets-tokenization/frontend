...
export default ({
    ...
    tableColumns = tableColumns.concat(Object.keys(columns).map((key) => {
        ...
        return {
            ...
            render: (value, item, columnKey, rowIndex, modal) => {
                const replaceBr = 
                    value && typeof value === 'string' && !elementSchema.multiline && !elementSchema.mode
                        ? value.replace(/(?:\r\n|\r|\n)/gi, ' ')
                        : value;

                const parentValueIndexed = parentValue[name] && parentValue[name][rowIndex];
    ...
    return {
        ...
        CustomToolbar: rows ? null : props => (limit ? null : <AddItemButton readOnly={readOnly} {...props} t={t} />),
