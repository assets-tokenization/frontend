...
class TableElement extends React.Component {
    ...
    getItems = () => {
        ...
        if (rows && Array.isArray(rows)) {
            return rows.map((row, index) => values[index] || emptyValues[(items.properties || items).type || 'object']);
        }

        return (values.length || allowEmpty) ? values : [emptyValues[(items.properties || items).type || 'object']];
    };

    render = () => {
        const { t, sample, description, items, actions, required, error, hidden, ...rest } = this.props;
