...
class Address extends React.Component {
  ...
  handleChangeStreet = async (selected) => {
    ...
    const newValue = selected
      ? (optionsArray || []).filter(({ id }) => selected.id.includes(id)).shift()
      : null;
  ...
  renderStreetControl = () => {
    ...
    return (
      <ElementContainer required={true} error={this.checkAdressError()} bottomSample={true}>
        <Select
          isLoading={loading}
  ...
  renderControl = (key) => {
    ...
    return (
      <SchemaForm
        ...
        onChange={
          key === 'ATU'
            ? handleChangeAdapter(value, this.handleChangeATU, false, template.jsonSchema).bind(
                null,
                key
              )
