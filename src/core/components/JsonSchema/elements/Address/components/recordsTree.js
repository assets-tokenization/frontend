...
import { requestRegisterKeyRecords, requestRegisterKeyRecordsFilter } from 'actions/registry';
...
const toOption = ({ id, stringified, data }) => ({ id, value: id, label: stringified || data.stringified || data.name, ...data });
...
class Address extends React.Component {
    ...
    getSchemaPath = () => {
        ...
        return path.join('.properties.').split('.').filter(el => el !== name).join('.').replace(/properties.\d+/g, 'items');
    };
    ...
    getFilters = () => {
        ...
        return `data[atuParentId]=[${atuParentId}]&sort[data.name]=asc&offset=${page * 10}&limit=${10}&strict=true&search=${streetName.trim()}`;
    };
    ...
    getOptions = async () => {
        ...
        const options = await importActions.requestRegisterKeyRecords(keyId, this.getFilters(), true);

        actions.setBusy(false);

        const optionsArray = uniqbyValue((savedOptions || []).concat(options.map(toOption)));
    ...
    setMergetControlTemplate = () => {
        ...
        const schemaPath = `jsonSchema.properties.${stepName}.properties.${path.length > 1 ? `${this.getSchemaPath()}.` : ''}${name}`;
    ...
    checkATUvalues = newValue => {
        ...
        const atuSelected = Object.values(ATU).filter(Boolean).map(({ atuId }) => atuId).filter(Boolean);
    ...
    checkAdressError = () => {
        ...
        const filter = errors.filter(({ path: filterPath }) => filterPath.indexOf(itemPath) !== -1);
    ...
    handleChangeStreet = async (selected) => {
        ...
        const newValue = selected ? (optionsArray || []).filter(({ id }) => selected.id.includes(id)).shift() : null;
    ...
    updateRequired = async () => {
        ...
        objectPath.set(rootDocument.data, `${this.getControlPath()}.required`, required);
        if (required === false) this.setDefaultATUProps();
    }

    updateHidden = async () => {
        const { rootDocument, hidden } = this.props;
        objectPath.set(rootDocument.data, `${this.getControlPath()}.hidden`, hidden);
    ...
    renderControl = key => {
        ...
        return (
            <SchemaForm
                ...
                onChange={
                    key === 'ATU'
                        ? handleChangeAdapter(value, this.handleChangeATU, false, template.jsonSchema).bind(null, key)
    ...
    componentDidUpdate = prevProps => {
        ...
        if (JSON.stringify(prevProps.value) !== JSON.stringify(this.props.value)) {
            ...
            objectPath.set(rootDocument.data, this.getControlPath(), removeEmptyFields(value));
        }
    };
