export default ({ required }) => ({
    ...
    'properties': {
        'ATU': {
            ...
            'properties': {
                ...
                'district': {
                    ...
                    'sample': "<div style='background: #FFF4D7; padding: 10px 15px 10px 15px'>Назва має бути актуальною (враховуючи перейменування)</div>"
                },
                'city': {
                    'keyId': 411,
        ...
        'building': {
            ...
            'checkValid': [
                {
                    'isValid': '(value, step, document, parent) => value && value.length <= 10',
                    'errorText': 'Може містити не більше, ніж 10 символів'
                },
                {
                    'isValid': '(value, step, document, parent) => value && !/[-]{2,}/.test(value) && !/[ ]{2,}/.test(value) ',
                    'errorText': 'Не може містити більше одного дефісу чи пробілу підряд'
                }
            ],
            'checkRequired': '(value, step, document, parent, propertySchema, parentSchema) => parent.required',
            'checkHidden': '(value, step, document, parent) => parent.hidden'
        },
        'korpus': {
            'type': 'string',
            ...
            'checkHidden': "(value, step, document, parent) => (parent && parent.isPrivateHouse && parent.isPrivateHouse[0] === 'приватний будинок') || parent.hidden"
        },
        'isPrivateHouse': {
            'type': 'array',
        ...
        'apt': {
            ...
            'checkRequired': "(value, step, document, parent) => !(parent && parent.isPrivateHouse && parent.isPrivateHouse[0] === 'приватний будинок') && parent.required",
            'checkHidden': "(value, step, document, parent) => (parent && parent.isPrivateHouse && parent.isPrivateHouse[0] === 'приватний будинок') || parent.hidden"
        },
        'index': {
            'type': 'string',
