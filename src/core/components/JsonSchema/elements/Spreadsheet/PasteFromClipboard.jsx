...
const PasteFromClipboard = ({ t, data, menuPosition, handleClose, onCellsChanged }) => {
    const [cell, setCell] = React.useState({});

    React.useEffect(() => {
        if (menuPosition && (cell.i !== menuPosition.i || cell.j !== menuPosition.j)) {
    ...
    const handlePaste = async () => {
        ...
        const copyValues = copyValue.split(/\r\n|\n|\r/).map(row => row.split('\t'));
        ...
        copyValues.forEach((newRow, newRowIndex) => newRow.forEach((newCellValue, newCellIndex) => {
