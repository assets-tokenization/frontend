...
export const arrayToData = (array = [], items = {}) => array
    .filter(row => Array.isArray(row) && row.length)
    .map(row => Object.keys(items.properties)
        .reduce((acc, propertyName, index) => {
            let value = row[index] ?? '';
            const property = items.properties[propertyName];

            if (value instanceof Date) {
                value = humanDateFormat(new Date(value.getTime() + 2 * 60 * 60 * 1000), 'DD.MM.YYYY');
            }
...
export const output = (onChange, value, items = {}, useCellChangeHandler = false) => (changes, additions) => {
    const props = Object.keys(items.properties || {});
    let data = [].concat(value && JSON.parse(JSON.stringify(value))).filter(Boolean);
    [].concat(changes, additions)
        ...
        .forEach(({ row, col, value: newValue }) => {
            ...
            if (property && property.control === 'currency.input') {
                newValue = stringToNumber(newValue).toFixed(property.decimalPlaces || 2);
            }
