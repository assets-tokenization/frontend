...
const DataEditor = ({ classes, col, row, cell, items, path, name, readOnly, parentValue, onChange, onCommit, onCellChange, cellRef, ...rest }) => {
    ...
    const [editorValue, setEditorValue] = useStickyState(cell.value, path.concat(row, propName).join('.'), 1000);
    let jsonValue;
    ...
    return (
        ...
        >
            <SchemaForm
                ...
                onChange={
                    newValue => {
                        const dataValue = newValue instanceof ChangeEvent ? newValue.data : newValue
