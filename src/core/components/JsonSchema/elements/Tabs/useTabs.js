...
export const useTabs = ({
  ...
  const tabs = useMemo(() => Object.keys(properties).filter((tabName) => {
  ...
  const handleChange = useCallback((event, activeTab) => onChange && onChange({
    ...(emptyHidden ? {} : value || {}),
  ...
  useEffect(() => {
    ...
    const hiddenValues = tabs.filter((tabName, index) => (index === activeTab ? false : !!value?.[tabName])).filter(Boolean);
