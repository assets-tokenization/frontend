...
class Popup extends React.Component {
  ...
  handleClickOpen = () => {
    ...
    this.setState({ open: true }, () => actions.clearErrors && actions.clearErrors(path.join('.')));
  };

  deleteItem = async () => {
    const { actions, rootDocument, stepName, path, handleDeleteCallBack } = this.props;
    ...
    objectPath.set(rootDocument.data[stepName], computedPath, undefined);
    ...
    return (
      <Fragment key={index}>
        {Object.keys(controlsNames).map((val, key) => {
          ...
          const [firstFilteredOption] = (controlsNames[val].options || []).filter(
            ({ id }) => Number(id) === Number(value[item][val])
          );
          ...
          return (
            <Fragment key={key}>
              ...
              <FieldValue>
                {controlsNames[val].options && value[item] && firstFilteredOption
                ...
                {typeof value[item][val] === 'object'
                  ? Object.keys(value[item][val]).map((option, fieldKey) => (
                      <FieldValue key={fieldKey}>
                        {value[item][val][option] && value[item][val][option].label}
                      </FieldValue>
                    ))
                  : null}
  ...
  renderSelectData = (item, index) => {
    ...
    return (
      <Fragment key={index}>
        ...
        {value[item] && !isArray && <FieldValue>{value[item].stringified}</FieldValue>}
      </Fragment>
    );
  };
  ...
  renderTreeSelectData = (item, index) => {
    ...
    return (
      <Fragment key={index}>
        <FieldName style={{ marginBottom: 8 }}>{properties[item].description}</FieldName>
        {value[item] && (
          <FieldWithBackGround>
            <FieldValue style={{ marginBottom: 0 }}>{value[item].name}</FieldValue>
          </FieldWithBackGround>
        )}
      </Fragment>
    );
  ...
  render = () => {
    const { hidden, required, row, noMargin, isAddButton, classes, addItemText } = this.props;
