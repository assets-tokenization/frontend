...
const PropertyList = ({ t, classes, schema, value, readOnly, onChange }) => {
  const [path, setPath] = React.useState([Object.keys(schema.properties)[0]].filter(Boolean));

  const activeProperty = objectPath.get(schema.properties, path.join('.properties.'));
  ...
  return (
    <div className={classes.root}>
      <div className={classes.tree}>
        <Scrollbar>
          <PropertyTree path={[]} active={path} properties={schema.properties} setPath={setPath} />
        </Scrollbar>
      </div>
