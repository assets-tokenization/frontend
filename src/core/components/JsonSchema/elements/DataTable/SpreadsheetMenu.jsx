...
const SpreadsheetMenu = ({ t, data, value, onChange, menuPosition, setMenuPosition, onCellsChanged }) => (
    <Menu
        open={Boolean(menuPosition && (menuPosition.row || navigator.clipboard.readText))}
    ...
    >
        ...
        {menuPosition && menuPosition.row ? (
            <CopyToClipboard
                ...
                text={menuPosition.row.map(({ value: strValue }) => strValue).join('\t') + '\n'}
                onCopy={() => setMenuPosition(null)}
            >
                <MenuItem>
                    <ListItemIcon>
