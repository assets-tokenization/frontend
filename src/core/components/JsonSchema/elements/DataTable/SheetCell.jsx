...
import FormControlMessage, { MESSAGE_TYPES } from 'components/JsonSchema/components/FormControlMessage';
...
const SheetCell = (props) => {
    ...
    const [message] = getMessages(schema, [].concat(stepName, path, row, propName), { ...props, value });

    const isList = React.useMemo(() => !schema.control && schema.type === 'string' && schema.options, [schema]);

    const isDropDownList = !editing && !isList && listControls.includes(schema.control);
    ...
    const sheetCell = (
        <td
            ...
            className={
                classNames(className, classes.cell, {
                    [classes.error]: !!error
                })
            }
            style={
        ...
        >
            ...
            >
                ...
                >
                    ...
                    {value ? <div className={classNames(classes.copier, 'copier')} dataRole="copier" dataValue={value} /> : null}
