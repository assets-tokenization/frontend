...
const DataEditor = ({ col, row, cell, items, path, name, readOnly, parentValue, onChange, onCommit, ...rest }) => {
    const properties = Object.keys(items.properties || {});
    const propName = properties[col];
    const schema = items.properties[propName];

    const [value, setValue] = useStickyState(cell.value, [rest.taskId, rest.stepName].concat(path).join(), 1000);
    ...
    return (
        <div style={{ zIndex: 1, width: '100%' }}>
            <SchemaForm
                ...
                schema={{
                    ...schema,
                    description: allowedDescriptionControls.includes(schema.control) ? schema.description : ''
                }}
                ...
                onChange={
                    (...path) => {
                        ...
                        let dataValue = newValue instanceof ChangeEvent ? newValue.data : newValue;
