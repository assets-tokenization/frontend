...
const awaitDelay = delay => new Promise((fulfill) => {
...
class Payment extends React.Component {
    ...
    parseResult = async(result) => {
        ...
        const isSuccess = (typeof processed !== 'undefined' && !!processed[processed.length - 1].status.isSuccess) || false;

        const alreadyPassed = (typeof value.processed !== 'undefined' && !!value.processed[value.processed.length - 1].status.isSuccess) || false;
