...
const PrevNavigationButton = ({ handlePrevClick, page }) => {
  ...
  return (
    <Button variant="contained" color="yellow" onClick={handlePrevClick} disabled={page === 1}>
...
const NextNavigationButton = ({ handleNextClick, page, pages }) => {
  ...
  return (
    <Button variant="contained" color="yellow" onClick={handleNextClick} disabled={page === pages}>
...
const NavigationPageLabel = withStyles({})(({ classes, page, pages }) => (
  <Typography
    variant="h6"
    className={cx(classes.pageLabel, page === pages && classes.pageLabelWithoutButtons)}
  >
    {page} / {pages}
  </Typography>
));
