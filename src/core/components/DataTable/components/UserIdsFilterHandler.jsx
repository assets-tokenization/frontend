...
class UserIdsFilterHandler extends FilterHandler {
    ...
    renderChip = () => {
        ...
        const userListStringified = userList.slice(0, 3).map(user => {
            if (user.userId) {
    ...
    checkUserNames = async () => {
        ...
        const notStoredUsers = notListedUsers.filter(id => !results.find(({ userId }) => userId === id));
    ...
    handleSearch = () => {
        ...
        waiter.addAction('UserIdsFilterHandler', async () => {
            this.setState({ searchResults: null });
        ...
        return (
            <Paper elevation={0} className={classes.root}>
                <TextField
                    ...
                    onChange={({ target: { value: newValue } }) => this.setState({ search: newValue }, this.handleSearch)}
                    type={type} />
                <DataTableStated
                    ...
                    onRowClick={(user) => this.setState({ selected: selected.filter(id => id !== (user.userId || user)) })}
                />
                {search ? (
                    <>
                        <DataTableStated
                            ...
                            data={searchResults && searchResults.filter(({ userId }) => !selected.includes(userId))}
