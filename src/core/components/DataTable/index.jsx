...
import { Table, TableBody, TableContainer, TableCell, TableRow, ImageList, Divider } from '@mui/material';
...
class DataTable extends React.Component {
    ...
    renderBody = () => {
        const { maxTextRows, t, data, groupBy, columns, hiddenColumns, rowsSelected, hightlight, checkable, actions, onRowClick, cellStyle, editPopupMode, cellColor, hover, hiddenRows, darkTheme } = this.props;
        ...
        const renderRow = (row, key) => {
            ...
            return (
                <DataTableRow
                    ...
                    onClick={
                        onRowClick && !disableClick
                            ? onRowClick.bind(this, row, key)
                            : null
                    }
                ...
                <TableBody>
                    {
                        groups.map(group => (
                            <CollapsedTableRows
                                ...
                                renderRow={renderRow}
                                colSpan={(columns || []).length + (checkable ? 1 : 0)}
                                data={data.filter(({ [groupBy]: filterData }) => [].concat(filterData).includes(group))}
                            />
                        ))
                    }
                    <CollapsedTableRows
    ...
    renderCards = () => {
        ...
        return (
            <ImageList className={classes.cardContainer}>
                {data.map(renderCard)}
            </ImageList>
        );
    };
    ...
    renderToolBar = props => {
        const { groupBy, className } = this.props;
        const { view, grouping, fullscreen } = this.state;
        const { classes, ...clearProps } = this.props;
    ...
    renderTable = () => {
        const { t, classes, actions, count, page, rowsPerPage, controls, fixedTable, stickyHeader, maxHeight, noMargin, filters, presets, filterHandlers, darkTheme, loading } = this.props;
        const { view, fullscreen } = this.state;

        return (
            <>
                {controls.toolbar && !controls.bottomToolbar && !maxHeight ? this.renderToolBar() : null}
                {controls.presets ? <DataTableFilterPresetBar actions={actions} filters={filters} presets={presets} filterHandlers={filterHandlers}/> : null}
                ...
                >
                    {view === 'table' ? (
                        <TableContainer
                            ...
                            style={{ maxHeight: fullscreen ? 'calc(100vh - 130px)' : maxHeight || 'unset' }}
                        >
                            <Table
                                stickyHeader={stickyHeader}
                                className={classNames(classes.table, { [classes.fixedTable]: fixedTable })}
                            >
                    ...
                    {controls.toolbar && controls.bottomToolbar ? this.renderToolBar({ bottomToolbar: true }) : null}
    ...
    render = () => {
        const { dragEvents, tableProps = {}, tableTitle } = this.props;
        const { fullscreen } = this.state;
