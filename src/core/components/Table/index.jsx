...
const getTotal = list =>
    ((list && list.meta && list.meta.pagination && list.meta.pagination.total) ? list.meta.pagination.total : list.length);
...
const CustomTable = ({
    ...
    const { tableFields, checkbox, pagination: needPagination, selectAllCheckbox } = fields;
    ...
    const listWithPermissions = checkbox ? list.map(item => ({ ...item, havePermissions: isHavePermission(item) })) : list;
    const havePermissionsList = listWithPermissions.filter(({ havePermissions }) => havePermissions);
    const needAllCheckbox = checkbox && selectAllCheckbox && ownerList;
    const showPagination = dataSource.count < total && total > 0;
    const itIsFavorite = item => (('id' in item) ? !!favorites.find(({ id }) => id === item.id) : false);
    return (
        <Table className={cx(classes.table, classes.mobileBlock)} id={setId('table')}>
            <TableHead
                id={setId('table-head')}
                className={needAllCheckbox ? classes.mobileBlock : classes.onlyBigScreen}
            >
                        ...
                        >
                            {needAllCheckbox &&
                                <Checkbox
                                    ...
                                    indeterminate={checked.length > 0 && checked.length < havePermissionsList.length}
            ...
            <TableBody id={setId('table-body')} className={classes.mobileBlock}>
                {listWithPermissions && listWithPermissions.map((data, index) => (
                    <TableRow
                    ...
                    >
                        {checkbox && ownerList &&
                            ...
                            >
                                ...
                                {data.resharing &&
                                    <Icon className={cx(classes.icon, checked.includes(data.id) && classes.checkbox)}>
                                        supervisor_account
                                    </Icon>
                                }
                            </TableCell>
                        ...
                        {tableFields.map(item => (
                            <TableCell
                                onClick={onCheckItem(needFullData ? data : data.id || data.attachId, data)}
                                id={setId(`table-cell-${item.key}`)}
                                key={`table-cell-${item.key}`}
                                className={getClasses(classes, item, ownerList)}
                                title={item.key === 'status' || item.key === 'state' || item.key === 'icon' ? '' : getText(data, item.key)}
                                numeric={!!data.numeric}
                            >
                                {!item.classNames.includes('onlyMobile') && !item.classNames.includes('onlyBigScreen') && item.title &&
                                    <span className={cx(classes.onlyMobileText, classes.onlyMobile)}>
                                        {`${t(item.title)}: `}
                                    </span>
                                }
