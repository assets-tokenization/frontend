...
const PreviewButton = ({
    ...
    url
        ...
        <ButtonBase
            ...
            onClick={alwaysPreview && !loading && preview && !(format === 'unknown' || format === 'binary') ? toggleDialog : handleDownload}
            id={setId('button-base')}
            title={format === 'unknown' || format === 'binary' ? `${t('UNKNOWN_FORMAT')} ${format === 'binary' ? t('DOWNLOAD_ONLY') : t('NOT_SUPPORTED')}` : t('PREVIEW')}
        >
            {loading ? (
                <CircularProgress
            ...
            {(format === 'binary' || (!(doc && format === 'pdf') && !loading && (!preview || !showPreview))) ? (
                <Icon
                    className={classes.fileImage}
