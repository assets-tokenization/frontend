...
const fields = {
    ...
    tableFields: [
        ...
        { key: 'icon', title: 'PREVIEW', classNames: ['cell', 'textRight'], grid: [5, 7, 1, 4] }
    ]
};
...
class AttachTable extends Component {
    ...
    openDialog = attach => () => {
        ...
        if (format === 'binary' || format === 'unknown') {
            ...
            (activeAttach && activeAttach.attachId === attach.attachId) &&
            (this.state.name === name)
        ) {
            this.setState({openDialog: true});
        } else {
            this.setState({...initalState, activeAttach: attach}, () =>
                this.setDoc(true));
        }
        this.setState({name});
    }
    ...
    handleDownload = attach => () =>
        this.props.handleDownload(attach)().then((file) => {
            if (typeof file === 'string') {
