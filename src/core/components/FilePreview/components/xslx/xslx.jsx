...
const ExcelRenderer = (file, callback) => new Promise((resolve) => {
        ...
        return (
            <>
                <div className={'ExcelTableTabsWrapper'}>
                    {
                        keys.length > 1 ? keys.map((name, index) => (
                            <div
                                key={name}
                    ...
                    keys.map((name, index) => {
                        ...
                        return (
                            <table
                                ...
                                key={rows.length}
                                style={{ display: (index === 0 && !active) || active === index ? 'table' : 'none' }}
                            >
                                <tbody>
                                    <tr>
                                        <th />
                                        {cols.map((c) => <th key={c.key} className={c.key === -1 ? 'heading' : ''}>{c.key === -1 ? '' : c.name}</th>)}
                                    </tr>
                                    {
                                        rows.map((r, i) => (
                                            <tr key={i}>
                                                <td key={i} className={'heading'}>{i + 1}</td>
                                                {
                                                    cols.map((c, j) => {
                                                        const isMerging = (merges || []).filter(m => m.s.r === i && m.s.c === j).pop();
                                                        const isMerged = (merges || []).filter(m => between(i, m.s.r, m.e.r) && between(j, m.s.c, m.e.c)).pop();
