...
class SelectFilesDialog extends React.Component {
  ...
  onDrop = (files, rejected) => {
    ...
    if (rejected.length) {
      ...
      if (maxSize < rejectedFile.size) {
        error = t('ERROR_SIZE_FILE', { maxSize: (maxSize / 1048576).toFixed(1) });
      } else {
  ...
  handleChange = ({ target }) =>
    this.setState({ name: target.value, error: false, showEmptyNameError: false });
  removeFile = () => this.setState({ file: null });

  uploadFile = async () => {
    const { t, uploadAction, onUpload, fileName } = this.props;
    const { file, name } = this.state;
    this.setState({ error: null, busy: true, openVerifiedDialog: false }, async () => {
      try {
        ...
        if ((!(link || fileLink) && fileName === 'doc') || (!attachId && fileName !== 'doc')) {
          throw new Error(t('ERROR_UPLOADING_FILE'));
        }
  ...
  render() {
    const { t, classes, open, accept, maxSize, fileName, setId, fileLimit, claim } = this.props;
    const { file, showEmptyNameError, busy, name, error, openVerifiedDialog } = this.state;
    const getCurrentTitle = (title) => t(`${title}${fileName ? `_${fileName.toUpperCase()}` : ''}`);
    ...
    return (
      ...
      >
        ...
        >
          {error && (
            <Typography className={classes.flex} align="left" color="error" id={setId('error')}>
              {error}
            </Typography>
          )}
          <Button
