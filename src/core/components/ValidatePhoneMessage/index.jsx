...
class ValidatePhoneMessage extends React.Component {
    componentWillReceiveProps({ handleClose, auth: { info: { valid } } }) {
    ...
    handleValidate = () => {
        const { t, actions, auth: { info: { phone } } } = this.props;
    ...
    handleChangeCode = ({ target: { value } }) => this.setState({ code: value, error: null });

    handleOpenDialog = () => this.setState({ open: true }, () => {
        const { actions, auth: { info: { phone } } } = this.props;
        actions.sendSMSCode(phone);
    });

    render() {
        const { open, validated, error } = this.state;
        const { t, classes, handleClose, auth: { info } } = this.props;
        ...
        return (
            <Fragment>
                ...
                >
                    <DialogTitle id="form-dialog-title">{t('ValidatePhoneTitle')}</DialogTitle>
                    <DialogContent>
                        {validated ? (
                            <DialogContentText>
                    ...
                    <DialogActions>
                        {validated ? (
                            <Button onClick={() => this.setState({ open: false }, handleClose)}>{t('Close')}</Button>
                        ) : (
                                <Fragment>
