...
import objectProxy from 'services/api/objectProxy';
import requestDataTypes from 'services/api/requestDataTypes';
...
async function createRequest(request, action, dispatch, payload, options) {
  ...
  try {
    ...
    const errors = []
      .concat(responseBody.error, responseBody.errors, responseBody.err)
      .filter(Boolean);
