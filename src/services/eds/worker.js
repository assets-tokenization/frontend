...
class Signer {
  ...
  setDefaultSettings(settings) {
    ...
    this.signer.SetRuntimeParameter(EU_CHECK_CERT_CHAIN_ON_SIGN_TIME_PARAMETER, true);
  }
...
const actions = {
  ...
  ReadPrivateKey: (key, password) =>
    signer.signer.ReadPrivateKeyBinary(keyToUint8Array(key), password),
  ...
  ProtectDataByPassword: (data, password, asBase64String) =>
    signer.signer.ProtectDataByPassword(keyToUint8Array(data), password, asBase64String),
  SetProxySettings: (settings) => signer.SetProxySettings(settings),
  GetDataFromSignedFile: (file) =>
